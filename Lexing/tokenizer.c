...
size_t SelectToken(char* buffer,
  ...
  if(IS_COMMENT) {
    while (size_read < size && buffer[size_read] != '\n') {
      size_read++;
    }
    if (size_read < size && buffer[size_read] == '\n') {
        IS_COMMENT = 0;
        size_read++;
    }
    return size_read;
  ...
  */
  ...
  if (buffer[size_read] == '+') {  // + and ++
    ...
    /* YOUR CODE HERE */
    char char_read = buffer[size_read + 1];
    if (size_read + 1 == size) {
      return size_read;
    }
    else if ((char_read == '\\') && (size_read + 3 < size) && 
    ...
    else if (isprint(char_read) && (size_read + 2 < size) && (buffer[size_read + 2] == '\'')) {
      size_read += 3;
      t = create_token(filename);
      t->linenum = *linenum;
      t->type = TOKEN_CHARACTER;
      t->data.character = char_read;
    }
    ...
    while (size_read + int_len < size && search) {
      if (is_digit(buffer[size_read + int_len])) {
         ...
         */
        /* YOUR CODE HERE */
        if (int_len == 1 || buffer[size_read] != '0') {
          t = create_token(filename);
          t->type = TOKEN_INTEGER;
          t->linenum = *linenum;
          char** end = NULL;
          t->data.integer = strtol(buffer, end, 10);
          size_read += int_len;
        }
        /* FIXME IM NOT CORRECT. (generic string) */
        else {
          int total = generate_generic_error(&t, buffer, size_read, size, *linenum,
                                          filename);
          if (total == 0) {
    ...
    while (size_read + id_len < size && search) {
      if (is_identifier_component(buffer[size_read + id_len])) {
        ...
        if (type != TOKEN_ERR) {
        ...
        else if (is_valid_identifier(token_contents)) {  // (FIX ME) Variable/Function name
          /* Handle identifiers */
          /* YOUR CODE HERE */
          t = create_token(filename);
          t->linenum = *linenum;
          ...
          t->data.identifier =
            (char*)malloc(sizeof(char) * strlen(token_contents) + 1);
          for (int i = 0; i < strlen(token_contents); i++) {
            t->data.identifier[i] = token_contents[i];
          }
          t->data.identifier[strlen(token_contents)] = '\0';
          size_read += id_len;
        }
        else {
          /* Errors */
