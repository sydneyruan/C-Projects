...
int is_valid_identifier(char* str) {
  ...
  str++;
  while (*str) {
    if(!is_identifier_component(*str))
      return 0;
    str++;
  }
  return 1;
...
char* str_concat(char** strings, size_t count) {
  ...
  char* new_str = malloc(0);
  char* curr_str = malloc(0);

  while(size > 0) {
    curr_str = new_str;
    new_str = malloc(strlen(curr_str) + strlen(*pointers));
    strcat(new_str, curr_str);
    strcat(new_str, (*pointers));
    free(curr_str);
    pointers++;
    size--;
  }
  size_t last_char = strlen(new_str);
  curr_str = new_str;
  new_str = malloc(last_char + 1);
  strcat(new_str, curr_str);
  new_str[last_char] = '\0';
  free(curr_str);
  return *strings;
